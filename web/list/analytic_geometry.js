...
function distance_parallel() {
    ...
    input_c2 = Number(document.forms["distance_parallel_form"]["distance_parallel_c2"].value)
    input_a = Number(document.forms["distance_parallel_form"]["distance_parallel_a"].value)
    input_b = Number(document.forms["distance_parallel_form"]["distance_parallel_b"].value)
    distance_parallel = Math.abs(input_c1 - input_c2) / (input_a ** 2 + input_b ** 2) ** 0.5
    //| C2 - C1 | / root(A^2 + B^2)
...
function slope2point() {
    ...
    //d(X) / d(Y), d = delta
    document.getElementById("slope2point_output").value = slope2point_value
}
...
function point_center_3() {
    ...
    input_x1 = Number(document.forms["point_center3_form"]["point_center3_x1"].value)
    input_x2 = Number(document.forms["point_center3_form"]["point_center3_x2"].value)
    input_x3 = Number(document.forms["point_center3_form"]["point_center3_x3"].value)
    input_y1 = Number(document.forms["point_center3_form"]["point_center3_y1"].value)
    input_y2 = Number(document.forms["point_center3_form"]["point_center3_y2"].value)
    input_y3 = Number(document.forms["point_center3_form"]["point_center3_y3"].value)
    point_center_x = (input_x1 + input_x2 + input_x3) / 3
    point_center_y = (input_y1 + input_y2 + input_y3) / 3
    //((x1 + x2 + x3) / 3, (y1 + y2 + y3) / 3)
    document.getElementById("point_center3_x_output").value = point_center_x
    document.getElementById("point_center3_y_output").value = point_center_y
    //answer is (point_center_x, point_center_y) like (0, 0) or (3, 2)
}
...
//part of circle dont do it

//swap + to - and - to +
function swap_plus_minus(x) {
    x = String(x)
    if (x[0] == "-") {
        y = x.replace("-", "")
    }
...
function circle_position() {
    input_h = Number(document.forms["circle_position"]["circle_position_h"].value)
    input_k = Number(document.forms["circle_position"]["circle_position_k"].value)
    input_r = Number(document.forms["circle_position"]["circle_position_r"].value)
